...
# Run stage
#
# Use a JRE runtime as a parent image to make the Image smaller than an image using the JDK.
...
COPY --from=buildstage $APP_HOME/build/libs/CycloCrossLeagueManager-0.0.1-SNAPSHOT.jar $APP_HOME/CycloCrossLeagueManager.jar
# Set the working directory to /app, so we don't need to prefix the CMD Layer with /app.
WORKDIR $APP_HOME
# Expose port 8080
EXPOSE 8080
# Start the Spring Boot app
CMD ["java", "-jar", "CycloCrossLeagueManager.jar"]